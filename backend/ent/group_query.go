...
func (_q *GroupQuery) QuerySupplier() *SupplierQuery {
...
func (_q *GroupQuery) WithSupplier(opts ...func(*SupplierQuery)) *GroupQuery {
	query := (&SupplierClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withSupplier; query != nil {
		if err := _q.loadSupplier(ctx, query, nodes, nil,
			func(n *Group, e *Supplier) { n.Edges.Supplier = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadSupplier(ctx context.Context, query *SupplierQuery, nodes []*Group, init func(*Group), assign func(*Group, *Supplier)) error {
...
func (_q *GroupQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withSupplier != nil {
			_spec.Node.AddColumnOnce(group.FieldSupplierID)
		}
	}
