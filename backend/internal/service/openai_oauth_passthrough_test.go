...
func TestOpenAIGatewayService_APIKeyPassthrough_Stream504HTMLTriggersFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", bytes.NewReader(nil))
	...
	requestBody := []byte(`{"model":"gpt-5.6-sol","stream":true,"input":"hello"}`)
	const upstreamBody = `<!DOCTYPE html><html><head><title>upstream.example | 504: Gateway time-out</title></head><body><h1>Gateway time-out</h1><span class="code-label">Error code 504</span></body></html>`
	body := &passthroughCloseTrackingReadCloser{Reader: strings.NewReader(upstreamBody)}
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusGatewayTimeout,
		Header: http.Header{
			"Content-Type": []string{"text/html; charset=UTF-8"},
			"X-Request-Id": []string{"rid-api-key-stream-504"},
		},
		Body: body,
	}}
	svc := &OpenAIGatewayService{
	...
	require.Equal(t, "rid-api-key-stream-504", failoverErr.ResponseHeaders.Get("x-request-id"))
	require.False(t, c.Writer.Written(), "upstream HTML must not be committed to the downstream response")
	require.Zero(t, rec.Body.Len(), "downstream response body must remain empty for outer failover handling")
	...
	require.True(t, event.Passthrough)
	require.Equal(t, http.StatusGatewayTimeout, event.UpstreamStatusCode)
	require.Equal(t, account.ID, event.AccountID)
	require.Equal(t, upstreamBody, event.Detail)
	require.Equal(t, upstreamBody, event.UpstreamResponseBody)
}
