...
async function load() { try { const [s,w,c,r] = await Promise.all([adminSupplierAPI.list(),adminSupplierAPI.withdrawals(),adminSupplierAPI.settings(),adminSupplierAPI.resourceRequests()]); items.value=s.items || []; withdrawals.value=w.items || []; resourceRequests.value=r.items || []; Object.assign(settings,c) } catch (e) { error.value=e instanceof Error ? e.message : '供应商数据加载失败' } }
...
async function unfreeze(item:Supplier){if(!confirm(`确认解除 ${item.name} 的冻结状态？`))return;await adminSupplierAPI.unfreeze(item.id);await load()}
...
async function reviewResource(id:number,approved:boolean){const note=approved?'':(prompt('驳回原因')||'');await adminSupplierAPI.reviewResourceRequest(id,approved,note);await load()}
onMounted(load)
</script>
